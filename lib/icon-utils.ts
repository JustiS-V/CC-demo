  ...
  return Object.entries(ICON_REGISTRY)
    .filter(([name, metadata]) => {
      return (
        ...
        metadata.keywords.some(keyword => keyword.toLowerCase().includes(lowercaseQuery)) ||
...
export const validateSvgIcon = (svgContent: string): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  if (!svgContent.includes('currentColor') && !svgContent.includes('fill="currentColor"')) {
...
export const generateIconsExport = (): string => {
  ...
  const imports = iconNames
    .map(name => `import ${toPascalCase(name)}Svg from '@/assets/icons/svg/${name}.svg';`)
    .join('\n');
